...
RESULTS_DIR = ROOT_DIR / "results" / "stage2_h3"
LEDGER_PATH = ROOT_DIR / "logs" / "stage2_h3_results_ledger.json"
...
def _common_rebalance_dates(frames: dict[str, pd.DataFrame]) -> list[pd.Timestamp]:
    ...
    if not common_dates:
        raise ValueError("No common dates across H3 universe")
...
def _build_panel(frames: dict[str, pd.DataFrame], rebalance_dates: list[pd.Timestamp]) -> pd.DataFrame:
    ...
    if panel.empty:
        raise ValueError("Failed to build H3 panel")
    return panel
...
def _top_k_weights(score_frame: pd.DataFrame, top_k: int = TOP_K) -> dict[pd.Timestamp, dict[str, float]]:
    ...
    for date, group in score_frame.groupby("Date"):
        ...
        weights[pd.Timestamp(date)] = {str(row.Ticker): weight for row in ordered.itertuples(index=False)}
    return weights
...
def _window_summary(
    *,
    ...
    model_weights = _top_k_weights(model_scores)
    momentum_weights = _top_k_weights(momentum_scores)
...
def _aggregate_variant(variant_name: str, model_family: str, windows: list[dict[str, object]]) -> dict[str, object]:
    ...
    return {
        ...
        "selection_rule": f"top_{TOP_K}_equal_weight",
...
def _write_report(path: Path, summary: dict[str, object], windows: list[dict[str, object]]) -> None:
    lines: list[str] = []
    lines.append("# Stage 2 H3 Results Report")
    lines.append("")
    lines.append(f"Date: {datetime.now().date().isoformat()}")
    lines.append("Hypothesis: H3 - Cross-Sectional Ranking")
    ...
    lines.append(f"- Selection rule: top-{TOP_K} equal weight")
    ...
    lines.append("> Relative ranking may be more learnable than absolute direction and may reveal durable relative-strength structure.")
    lines.append("")
    ...
    lines.append("| ------ | ------ | --------------------- | ---------------------- | ------------------- |")
    for item in windows:
        lines.append(f"| {item['window_num']} | {item['period']} | {item['asset_count_available']} | {item['rebalance_observations']} | {'pass' if item['sufficient'] else 'fail'} |")
    lines.append("")
...
def run_h3_sweep() -> dict[str, object]:
    ...
    if len(all_rebalance_dates) < 30:
        raise ValueError("Insufficient rebalance dates for H3")
    ...
    if not windows:
        raise ValueError("No H3 rolling windows generated")
    ...
    for variant_name, model_family in variant_specs:
        ...
        for window in windows:
            ...
            period_returns: dict[pd.Timestamp, dict[str, float]] = {}
            for date, group in test_panel.groupby("Date"):
                period_returns[pd.Timestamp(date)] = {str(row.Ticker): float(row.forward_simple_return) for row in group.itertuples(index=False)}

            momentum_scores = test_panel[["Date", "Ticker", "forward_simple_return"]].copy()
            momentum_scores["score"] = test_panel["trailing_momentum"].to_numpy(dtype=float)

            if model_family == "momentum_rank":
                model_scores = momentum_scores.copy()
            else:
                train_panel = panel[panel["Date"].isin(window["train_dates"])]
                model = _fit_model(model_family, seed=42)
                model.fit(train_panel[FEATURE_COLUMNS].to_numpy(dtype=float), train_panel["forward_log_return"].to_numpy(dtype=float))
                model_scores = test_panel[["Date", "Ticker", "forward_simple_return"]].copy()
                model_scores["score"] = model.predict(test_panel[FEATURE_COLUMNS].to_numpy(dtype=float))

            window_result = _window_summary(
            ...
            window_result["window_rows"] = len(test_panel)
            window_result["sufficient"] = bool(len(UNIVERSE) >= MIN_UNIVERSE_SIZE and len(test_dates) >= MIN_REBALANCE_OBSERVATIONS)
            variant_windows.append(window_result)

        summary = _aggregate_variant(variant_name, model_family, variant_windows)
        summary["run_id"] = f"h3-{variant_name}-{datetime.now().strftime('%Y%m%d-%H%M%S')}"
        summary["variant_windows"] = variant_windows
        summary["report_path"] = f"results/stage2_h3/stage2_h3_{variant_name}_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
        _write_report(ROOT_DIR / summary["report_path"], summary, variant_windows)
        ledger["variants"].append(summary)

    ledger["status"] = "complete"
    ledger["final_decision"] = "PROCEED_TO_OPTION_B" if any(variant["verdict"] == "PASS" for variant in ledger["variants"]) else "EXIT_STAGE_2"
    ledger["best_variant"] = max(ledger["variants"], key=lambda item: float(item.get("mean_net_benchmark_gap", -1e9)))["event_variant"]
...
def main() -> int:
    ledger = run_h3_sweep()
    print("=" * 80)
    print("STAGE 2 H3 SWEEP COMPLETE")
    print("=" * 80)
    ...
    for variant in ledger["variants"]:
        recent_gap = variant["variant_windows"][-1]["net_benchmark_gap"] if variant["variant_windows"] else 0.0
        print(f"- {variant['event_variant']}: verdict={variant['verdict']} | mean_gap={variant['mean_net_benchmark_gap']:+.4f} | recent_gap={recent_gap:+.4f} | rank_ic={variant['primary_ranking_metric']:+.4f}")
    return 0
