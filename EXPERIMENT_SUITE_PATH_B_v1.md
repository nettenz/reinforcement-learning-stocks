...
## HARD GATES FOR STAGING PROMOTION

```
✅ PROMOTE if:
   1. Tier 1 complete (3 tickers trained, 10 seeds each)
   2. ≥2 tickers with ≥2 active seeds each
   3. Top-2 tickers mean test Sharpe ≥ 0.25
   4. Ensemble framework passes all unit tests
   5. Walk-forward validation shows ensemble accuracy ≥ min(seed accuracy)
   6. Ensemble agreement rate ≥ 60%

❌ HOLD if:
   - Only 1 ticker meets readiness threshold
   - Ensemble integration tests fail
   - Walk-forward ensemble degrades performance vs individual seeds

🔄 PIVOT if:
   - AMD entropy sweep shows <25% active seeds even at ent=0.10
   - Ensemble voting logic doesn't match seed agreement in practice
   - Feature ablation shows news/TA is actively degrading performance
```
...
- [ ] Exp 1 complete, top-3 NVDA seeds identified
- [ ] Exp 2 complete, top-2 AAPL seeds identified
- [ ] Exp 3 complete, AMD status determined
- [ ] `src/ensemble.py` written and unit-tested
- [ ] `ensemble_config.json` locked (seed selections final)
- [ ] `src/trading_agent.py` passes integration tests
- [ ] Walk-forward validation shows ensemble >= individual seeds
- [ ] Staging package built in `staging/` directory
- [ ] ENSEMBLE_READY_SUMMARY.md signed off
